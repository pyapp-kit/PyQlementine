...
def test_animations_enabled_by_default(qapp: QApplication) -> None:
    style = QlementineStyle()
    assert style.animationsEnabled() is True


def test_set_animations_disabled(qapp: QApplication) -> None:
    style = QlementineStyle()
    style.setAnimationsEnabled(False)
    assert style.animationsEnabled() is False


def test_default_auto_icon_color(qapp: QApplication) -> None:
    style = QlementineStyle()
    assert style.autoIconColor() == AutoIconColor.None_


def test_set_auto_icon_color(qapp: QApplication) -> None:
    style = QlementineStyle()
...
def test_set_theme(qapp: QApplication) -> None:
    style = QlementineStyle()
    theme = Theme()
...
def test_trigger_complete_repaint(qapp: QApplication) -> None:
    """Just verify it doesn't crash."""
    style = QlementineStyle()
    style.triggerCompleteRepaint()


def test_theme_all_public_fields(qapp: QApplication) -> None:
...
def test_theme_changed_signal(qtbot: QtBot) -> None:
    style = QlementineStyle()
    theme = Theme()
    theme.borderRadius = 99.0
    with qtbot.waitSignal(style.themeChanged):
        style.setTheme(theme)


def test_animations_enabled_changed_signal(qtbot: QtBot) -> None:
    style = QlementineStyle()
